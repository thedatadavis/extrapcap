...
`manage_live_cli` is the provider-backed bridge: it matches registry entries only when both option legs are currently held, parses the OCC symbols, requests the current indicative quotes for the exact expiration, and records a skip on missing metadata/quotes. It defaults to `dry-run`; `paper-submit` is an explicit mode and remains paper-endpoint-only. It manages both core credit verticals and paper crash-protocol bearish debit verticals using their distinct exit rules.
