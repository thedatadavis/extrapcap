...
class RiskConfig(BaseModel):
    max_core_open_risk_pct: float = Field(0.70, gt=0, le=1)
    max_asymmetric_open_risk_pct: float = Field(0.15, gt=0, le=1)
    max_daily_loss_pct: float = Field(0.15, gt=0, le=1)
    max_drawdown_brake_pct: float = Field(0.25, gt=0, le=1)
    max_sector_concentration_pct: float = Field(0.40, gt=0, le=1)
    max_ticker_concentration_pct: float = Field(0.20, gt=0, le=1)
    max_asymmetric_trades: int = Field(10, gt=0)
    max_orders_per_symbol_per_day: int = Field(20, gt=0)
    intraday_cooldown_minutes: int = Field(0, ge=0)
...
class StrategyConfig(BaseModel):
    ...
    min_width_pct: float = Field(0.005, gt=0, lt=1)
    max_width_pct: float = Field(0.05, gt=0, lt=1)
